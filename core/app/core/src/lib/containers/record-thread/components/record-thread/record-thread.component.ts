...
export class RecordThreadComponent implements OnInit, OnDestroy, AfterViewInit {
  ...
  ngOnInit(): void {
    ...
    if (this.config.filters$) {
      this.subs.push(
        this.config.filters$.subscribe((filters) => {
          this.store
            .setFilters(filters)
            .pipe(take(1))
            .subscribe(() => {
    ...
    this.listActionAdapter = this.actionAdapterFactory.create(
      this.store,
      this.config
    );
  }
  ...
  protected initPresetFieldsMapping() {
    ...
    this.subs.push(
      this.config.presetFields$.subscribe((presetFieldValues) => {
  ...
  protected initDataSubscription(): void {
    this.subs.push(
      this.store.stores$.subscribe((records) => {
